# ChatGPT Apps SDK Next.js Starter  (deployed to chatgpt-apps-sdk-nextjs-starter-tau.vercel.app)
...
- **Example tools**:
  - `show_content`: demo tool that returns a name and renders the widget
  - `get_weather`: returns current conditions + today's forecast for a city using **Open‑Meteo** (**no API key/auth**)
...
## Weather Tool (No API Key)

This starter includes a weather tool you can invoke from ChatGPT:

- **Tool name**: `get_weather`
- **Input**: `{ "city": "Tokyo" }`
- **API**: Open‑Meteo geocoding + forecast (free, no authentication required)
- **UI**: The widget renders a small weather card on the homepage template when the tool returns `structuredContent.kind = "weather"`.
